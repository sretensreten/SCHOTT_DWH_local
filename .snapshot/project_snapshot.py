...
DEFAULT_EXCLUDED_DIR_NAMES = {"snapshot", ".git", "node_modules", "__pycache__", ".pytest_cache", ".mypy_cache", ".ruff_cache", ".venv", "venv", "env"}
DEFAULT_EXCLUDED_FILE_NAMES = {".gitkeep"}
MARKDOWN_ESCAPES_TO_REPAIR = {r"\_": "_", r"\*": "*", r"\[": "[", r"\]": "]", r"\(": "(", r"\)": ")", r"\{": "{", r"\}": "}", r"\#": "#", r"\+": "+", r"\-": "-", r"\.": ".", r"\!": "!", r"\|": "|", r"\>": ">"}
...
def discover_configs(project_root):
    ...
    return [] if not config_dir.is_dir() else [(config_display_name(path), path) for path in sorted(config_dir.glob("*.json"))]
...
def normalize_relative_pattern(pattern):
    return str(pattern).strip().replace("\\", "/").lstrip("/")

def has_glob_pattern(pattern):
    return any(char in pattern for char in "*?[")

def path_matches_any_pattern(file_path, project_root, patterns):
    if not patterns:
        return False
    ...
    for pattern in patterns:
        ...
        if not pattern:
            continue
        if relative_path == pattern or fnmatch.fnmatch(relative_path, pattern):
            return True
        if "/" not in pattern and fnmatch.fnmatch(file_name, pattern):
            return True
    return False

def path_is_inside_any_folder(file_path, project_root, folder_patterns):
    if not folder_patterns:
        return False
    relative_path = file_path.relative_to(project_root).as_posix()
    for raw_pattern in folder_patterns:
        pattern = normalize_relative_pattern(raw_pattern).rstrip("/")
        if not pattern:
            continue
        ...
        while parent and parent != ".":
            if parent == pattern or fnmatch.fnmatch(parent, pattern):
                return True
            if "/" not in pattern and fnmatch.fnmatch(Path(parent).name, pattern):
                return True
...
def should_include_group_file(file_path, project_root, extensions, group_config, config):
    # Priority: exclude_files > include_files > exclude_folders > include_folders > extensions > global defaults.
    if path_matches_any_pattern(file_path, project_root, group_config.get("exclude_files", [])):
        return False
    if path_matches_any_pattern(file_path, project_root, group_config.get("include_files", [])):
        return True
    if path_is_inside_any_folder(file_path, project_root, group_config.get("exclude_folders", [])):
        return False
    extension_matches = not extensions or file_path.suffix.lower() in extensions
    if group_config.get("include_folders", []):
        return extension_matches and path_is_inside_any_folder(file_path, project_root, group_config.get("include_folders", []))
    return extension_matches and not should_exclude_path(file_path, project_root, config)
...
def read_csv_preview(file_path, max_rows):
    ...
    return "\n".join([f"-- CSV preview only: first {max_rows} data rows plus header when present.", f"-- Source file: {file_path.name}", "", *lines])
...
def collect_files_from_dir(project_root, dir_name, extensions, group_config, config):
    ...
    return [p for p in candidates if p.is_file() and should_include_group_file(p, project_root, extensions, group_config, config)]

def collect_pattern_files(project_root, include_patterns):
    files = []
    for pattern in include_patterns:
        pattern = normalize_relative_pattern(pattern)
        if not pattern:
            continue
        matched_files = project_root.glob(pattern) if has_glob_pattern(pattern) else [project_root / pattern]
        files.extend([p for p in matched_files if p.is_file()])
    return files
...
def collect_group_files(project_root, group_config, config):
    ...
    for file_path in collect_pattern_files(project_root, group_config.get("include_files", [])):
        if not path_matches_any_pattern(file_path, project_root, group_config.get("exclude_files", [])):
            resolved = file_path.resolve()
            if resolved not in processed:
                files.append(file_path)
                processed.add(resolved)
    for standalone_file in group_config.get("standalone_files", []):
        ...
        if file_path.is_file() and not path_matches_any_pattern(file_path, project_root, group_config.get("exclude_files", [])):
            resolved = file_path.resolve()
            if resolved not in processed:
                files.append(file_path)
                processed.add(resolved)
...
def get_agent_groups_config(config):
    agents_config = config.get("agents", {"enabled": True})
    if agents_config.get("groups"):
        return agents_config.get("groups", {})
    return {"agents": {"output_file_name": agents_config.get("output_file_name", f"{config.get('project_label', 'project')}_agents_snapshot.txt"), "include_files": agents_config.get("include_files", [f"**/{AGENTS_FILE_NAME}"]), "exclude_files": agents_config.get("exclude_files", []), "include_in_full_snapshot": agents_config.get("include_in_full_snapshot", True), "write_when_empty": agents_config.get("write_when_empty", False)}}

def get_agent_files(project_root, agent_group_config):
    processed = set()
    files = []
    for file_path in collect_pattern_files(project_root, agent_group_config.get("include_files", [f"**/{AGENTS_FILE_NAME}"])):
        if path_matches_any_pattern(file_path, project_root, agent_group_config.get("exclude_files", [])):
            continue
        resolved = file_path.resolve()
        if resolved not in processed:
            files.append(file_path)
...
def append_file_to_snapshot(project_root, file_path, group_config, toc_entries, snapshot_contents):
    ...
    snapshot_contents.append(f"\n\n## FILE: {display_path}\n```{file_path.suffix.lstrip('.')}\n{read_file_for_snapshot(file_path, group_config)}\n```\n")
...
def consolidate_project_files(config_argument=None, selected_groups=None, include_agents=None, clean=True):
    ...
    selected_groups = [g.strip() for g in selected_groups if g.strip()] if selected_groups else list(all_groups.keys())
    unknown_groups = [g for g in selected_groups if g not in all_groups]
    if unknown_groups:
    ...
    for group_name in selected_groups:
        group_config = all_groups[group_name]
        snapshot_file = write_group_snapshot(project_root, target_dir, project_label, group_name, group_config, collect_group_files(project_root, group_config, config))
        if snapshot_file is not None:
            generated_snapshot_files.append(snapshot_file)
            if group_config.get("include_in_full_snapshot", True):
                full_snapshot_files.append(snapshot_file)
    agents_config = config.get("agents", {"enabled": True})
    if include_agents and agents_config.get("enabled", True):
        for agent_group_name, agent_group_config in get_agent_groups_config(config).items():
            snapshot_file = write_group_snapshot(project_root, target_dir, project_label, agent_group_name, agent_group_config, get_agent_files(project_root, agent_group_config))
            if snapshot_file is not None:
                generated_snapshot_files.append(snapshot_file)
                if agent_group_config.get("include_in_full_snapshot", True):
                    full_snapshot_files.append(snapshot_file)
    full_snapshot_file = write_full_project_snapshot(target_dir, full_snapshot_file_name, full_snapshot_files)
...
def parse_args():
    ...
    parser.add_argument("--no-agents", action="store_true", help="Do not include agents/instruction snapshots.")
...
if __name__ == "__main__":
    args = parse_args()
    try:
        consolidate_project_files(config_argument=args.config, selected_groups=split_csv_argument(args.groups), include_agents=False if args.no_agents else None, clean=not args.no_clean)
